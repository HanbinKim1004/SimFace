...
def route_zeroPadding(M, route, warped):
...
def norm_crop(img, route, landmark, image_size=112, mode='arcface'):
    ...
    # normalize
    warped_normalize = cv2.normalize(warped, None, 0, 255, cv2.NORM_MINMAX)
    # equalizeHist
    warped_equalize = cv2.equalizeHist(warped)
    
    out_normalize = route_zeroPadding(M, route, warped_normalize)
    out_equalize = route_zeroPadding(M, route, warped_equalize)
